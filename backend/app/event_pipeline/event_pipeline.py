...
class EventPipeline(IEventPipeline):
    ...
    def _process_durable(self, event: Any) -> bool:
        ...
        delivers to registered consumers from the durable outbox.  Returns
        True on successful commit + scheduling; delivery is AT-LEAST-ONCE and
        independently retryable via the outbox.
        """
        ...
        # 6. Post-commit durable delivery to consumers.
        dd = self._delivery_dispatcher
        if dd is not None:
            try:
                dd.deliver_pending()
                ...
                _log.exception(
                    "post-commit delivery pass failed (events remain in "
